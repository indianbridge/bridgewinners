...
/**
 * Callback to adjust card size
 */
BW.updateCardImageSize = function() {
	console.log("received card-deck:updated");
...
BW.Problem.prototype.setupHandAndCardDeck = function() {
	...
	if ( width < 500 || height < 500 ) {
		...
		var event = "card-deck:updated.bw";
		$( document ).off( event );
		$( document ).on( event, BW.updateCardImageSize );				
	}
	var config = {
