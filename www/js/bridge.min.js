...
Bridge.Hand.prototype.addCard=function(a,b){Bridge._checkSuit(a,"In addCard");Bridge._checkRank(b,"In addCard");13===this.numCards&&Bridge._reportError(this.name+"'s Hand : already has 13 cards. Cannot add "+a+b,"In addCard");this.cards[a][b]&&Bridge._reportError(a+b+" is already assigned to "+this.direction+". Cannot add again","In addCard");if(this.deal){var c=this.deal.cards[a][b];c.isAssigned()&&Bridge._reportError(a+b+" is already assigned to "+c.getDirection()+". Cannot add again","In addCard")}this.cards[a][b]=
!0;this.deal&&this.deal.cards[a][b].assign(this.direction);this.numCards++;this.onChange("addCard",a+b)};
...
var d="",e=Bridge._generateClasses(c,["row","info"]),f=Bridge._getTag(a,e);if(a.show.direction||a.show.name){var d=d+Bridge._openTag(f,a,e,[]),l=[],m="",g="direction";if(a.show[g])var l=Bridge._generateClasses(c,["column",g]),m=Bridge._getTag(a,l),d=d+Bridge._openTag(m,a,l,[]),h=Bridge._generateClasses(c,["field",g]),k=Bridge._getTag(a,h),q=["data-"+g+"='"+this.get(g)+"'"],d=d+Bridge._openTag(k,a,h,q),d=d+this.get(g).toUpperCase(),d=d+Bridge._closeTag(k),d=d+Bridge._closeTag(m);g="name";if(a.show[g]){l=
Bridge._generateClasses(c,["column",g]);m=Bridge._getTag(a,l);d+=Bridge._openTag(m,a,l,[]);h=Bridge._generateClasses(c,["field",g]);k=Bridge._getTag(a,h);q=["data-"+g+"='"+this.get(g)+"'"];d+=Bridge._openTag(k,a,h,q);d+=this.get(g);if(a.show.countInHeader)var p="count",h=Bridge._generateClasses(c,["field",p]),k=Bridge._getTag(a,h),q=["data-"+p+"='"+this.get(p)+"'"],d=d+Bridge._openTag(k,a,h,q),d=d+("("+this.get(p)+")"),d=d+Bridge._closeTag(k);d+=Bridge._closeTag(k);d+=Bridge._closeTag(m)}d+=Bridge._closeTag(f)}var n=
"",r=0,h=a.alternateSuitColor?this.getAlternatingSuitOrder():Bridge.suitOrder;_.each(h,function(b){e=Bridge._generateClasses(c,["row",b]);f=Bridge._getTag(a,e);n+=Bridge._openTag(f,a,e,[]);g="suit";if(a.show[g]){l=Bridge._generateClasses(c,["column",g,b]);m=Bridge._getTag(a,l);n+=Bridge._openTag(m,a,l,[]);var d=Bridge._generateClasses(c,["field",g,b]),k=Bridge._getTag(a,d),p=["data-"+g+"='"+b+"'"];n+=Bridge._openTag(k,a,d,p);n+=Bridge.suits[b].html;n+=Bridge._closeTag(k);n+=Bridge._closeTag(m)}g=
"cards";if(a.show[g]){l=Bridge._generateClasses(c,["column",g,b]);m=Bridge._getTag(a,l);n+=Bridge._openTag(m,a,l,[]);var d=Bridge._generateClasses(c,["field",g,b]),k=Bridge._getTag(a,d),p=["data-suit='"+b+"'"],h="",q=0;_.each(Bridge.rankOrder,function(e){this.cards[b][e]&&(q++,d=Bridge._generateClasses(c,["field",g,b,e]),d.push(c+"-field-"+g+"-"+r),k=Bridge._getTag(a,d),h+=Bridge._openTag(k,a,d,["data-suit='"+b+"'","data-rank='"+e+"'","data-card='"+b+e+"'"]),a.show.text&&(h+=Bridge.ranks[e].html),
h+=Bridge._closeTag(k),r++)},this);0===q&&a.show.emptySuit&&a.show.text&&(h+="-");n+=h;n+=Bridge._closeTag(m)}n+=Bridge._closeTag(f)},this);a.show.countInContent&&(p="count",h=Bridge._generateClasses(c,["field",p]),k=Bridge._getTag(a,h),q=["data-"+p+"='"+this.get(p)+"'"],n+=Bridge._openTag(k,a,h,q),n+="("+this.get(p)+")",n+=Bridge._closeTag(k));d=Bridge._generateHTMLModule(a,d,n,"");a.containerID&&Bridge.embedHTML(a.containerID,d);!b&&a.registerChangeHandler&&(h=a.idPrefix+"-"+a.prefix,k="hand:changed."+
h,$(document).off(k),$(document).on(k,{config:_.cloneDeep(a),hand:this,id:h,event:k,direction:this.getDirection()},function(a,b){if(b===a.data.hand)if(0===$("#"+a.data.id).length){var c=a.data.event;$(document).off(c)}else b.toHTML(a.data.config,!0)}));return d};
...
["row","directions"]),d=Bridge._getTag(a,f),e=e+Bridge._openTag(d,a,f,[]),l=[],m="",g="direction",h=this.vulnerability;a.show[g]&&_.each(Bridge.directionOrder,function(b){l=Bridge._generateClasses(c,["column",g,b]);m=Bridge._getTag(a,l);e+=Bridge._openTag(m,a,l,[]);var d=Bridge._generateClasses(c,["field",g,b]);"b"!=h&&h!=b&&h!=Bridge.getPartner(b)||d.push("vulnerable");var f=Bridge._getTag(a,d);e+=Bridge._openTag(f,a,d,["data-"+g+"='"+b+"'"]);e+=b.toUpperCase();e+=Bridge._closeTag(f);e+=Bridge._closeTag(m)},
this);for(var e=e+Bridge._closeTag(d),k="",g="call",q=0,f=Bridge._generateClasses(c,["row","calls",q]),d=Bridge._getTag(a,f),k=k+Bridge._openTag(d,a,f,[]),p="w",f=!0;this.dealer!==p;)var f=!1,p=Bridge.getLHO(p),l=Bridge._generateClasses(c,["column",g,p]),m=Bridge._getTag(a,l),k=k+Bridge._openTag(m,a,l,[]),n=Bridge._generateClasses(c,["field",g,p]),r=Bridge._getTag(a,n),s=["data-"+g+"='' data-direction='"+p+"'"],k=k+Bridge._openTag(r,a,n,s),k=k+Bridge._closeTag(r),k=k+Bridge._closeTag(m);for(var u=
0;u<this.calls.length;++u){var t=this.calls[u],p=t.getDirection();"w"!==p||f||(k+=Bridge._closeTag(d),q++,f=Bridge._generateClasses(c,["row","calls",q]),d=Bridge._getTag(a,f),k+=Bridge._openTag(d,a,f,[]));l=Bridge._generateClasses(c,["column",g,p]);m=Bridge._getTag(a,l);k+=Bridge._openTag(m,a,l,[]);n=Bridge._generateClasses(c,["field",g,p]);r=Bridge._getTag(a,n);s=["data-"+g+"='"+t.toString()+"' data-direction='"+p+"'"];k+=Bridge._openTag(r,a,n,s);k+=t.toHTML();k+=Bridge._closeTag(r);k+=Bridge._closeTag(m);
f=!1}0<this.calls.length&&(p=Bridge.getLHO(p));!this.getContract().isComplete&&a.addQuestionMark&&("w"===p&&(q++,f=Bridge._generateClasses(c,["row","calls",q]),d=Bridge._getTag(a,f),k+=Bridge._closeTag(d),k+=Bridge._openTag(d,a,f,[])),l=Bridge._generateClasses(c,["column",g,p]),m=Bridge._getTag(a,l),k+=Bridge._openTag(m,a,l,[]),n=Bridge._generateClasses(c,["field",g,p]),r=Bridge._getTag(a,n),s=["data-"+g+"='?' data-direction='"+p+"'"],k+=Bridge._openTag(r,a,n,s),k=k+"?"+Bridge._closeTag(r),k+=Bridge._closeTag(m),
p=Bridge.getLHO(p));for(;"w"!==p;)l=Bridge._generateClasses(c,["column",g,p]),m=Bridge._getTag(a,l),k+=Bridge._openTag(m,a,l,[]),n=Bridge._generateClasses(c,["field",g,p]),r=Bridge._getTag(a,n),s=["data-"+g+"='' data-direction='"+p+"'"],k+=Bridge._openTag(r,a,n,s),k+=Bridge._closeTag(r),k+=Bridge._closeTag(m),p=Bridge.getLHO(p);k+=Bridge._closeTag(d);d=Bridge._generateHTMLModule(a,e,k,"");a.containerID&&Bridge.embedHTML(a.containerID,d);!b&&a.registerChangeHandler&&(k=a.idPrefix+"-"+a.prefix,q="auction:changed."+
k,$(document).on(q,{config:_.cloneDeep(a),auction:this,id:k,event:q},function(a,b){if(a.data.auction===b)if(0===$("#"+a.data.id).length){var c=a.data.event;$(document).off(c)}else b.toHTML(a.data.config,!0)}));return d};Bridge.getCallHTML=function(a){var b=a.length;if(1>b||2<b)return a;Bridge._checkBid(a);if(1===b)return Bridge.calls[a].html;if(2===b)return a[0]+Bridge.calls[a[1]].html};
...
Bridge._generateClasses(c,["column",g]),m=Bridge._getTag(a,l),d=d+Bridge._openTag(m,a,l,["colspan=13"]),h=Bridge._generateClasses(c,["field",g]),k=Bridge._getTag(a,h),q=[],d=d+Bridge._openTag(k,a,h,q),q="Assign / Unassign cards";a.show.activeHand&&(q="Assign Cards to Active Hand : "+Bridge.directions[this.getActiveHand()].name+" / Unassign from any hand");d=d+q+Bridge._closeTag(k);d+=Bridge._closeTag(m)}var d=d+Bridge._closeTag(f),p="",g="cards";_.each(Bridge.suitOrder,function(b){e=Bridge._generateClasses(c,
["row",g,b]);f=Bridge._getTag(a,e);var d=["data-suit='"+b+"'"];p+=Bridge._openTag(f,a,e,d);_.each(Bridge.rankOrder,function(e){var f=this.cards[b][e].getDirection();f||(f="");l=Bridge._generateClasses(c,["column",g,b,e]);f&&l.push("assigned");m=Bridge._getTag(a,l);d=["data-suit='"+b+"'","data-rank='"+e+"'","data-assignedto='"+f+"'"];p+=Bridge._openTag(m,a,l,d);h=Bridge._generateClasses(c,["field",g,b,e]);f?(h.push("assigned"),h.push("assigned-"+f)):h.push("unassigned");k=Bridge._getTag(a,h);p+=Bridge._openTag(k,
a,h,d);a.show.text&&(p+=Bridge.suits[b].html+Bridge.ranks[e].html);a.show.assignedTo&&f&&(p+="("+f.toUpperCase()+")");p+=Bridge._closeTag(k);p+=Bridge._closeTag(m)},this);p+=Bridge._closeTag(f)},this);var n="";a.show.reset&&(e=Bridge._generateClasses(c,["row","reset"]),f=Bridge._getTag(a,e),n+=Bridge._openTag(f,a,e,[]),l=[],m="",g="reset",l=Bridge._generateClasses(c,["column",g]),m=Bridge._getTag(a,l),n+=Bridge._openTag(m,a,l,["colspan=13"]),h=Bridge._generateClasses(c,["field",g]),k=Bridge._getTag(a,
h),q=[],n+=Bridge._openTag(k,a,h,q),n=n+"Reset/Clear Hand"+Bridge._closeTag(k),n+=Bridge._closeTag(m),n+=Bridge._closeTag(f));d=Bridge._generateHTMLModule(a,d,p,n);a.containerID&&(Bridge.embedHTML(a.containerID,d),$(document).trigger("card-deck:updated"));a.registerChangeHandler&&(q=a.idPrefix+"-"+a.prefix,$("#"+q).on("click","."+a.prefix+"-field-cards",{deal:this},function(a){try{var b=$(this).data("suit"),c=$(this).data("rank"),d=$(this).data("assignedto");d?a.data.deal.getHand(d).removeCard(b,
...
Bridge.Auction.prototype._createConciseBiddingBox=function(a){var b=this.getContract().allowedCalls(this.nextToCall),c=this.getSelectedLevel(),d=a.prefix,e=b.minimum_level,f="",l="level",m=Bridge._generateClasses(d,["row","level"]),g=Bridge._getTag(a,m),f=f+Bridge._openTag(g,a,m,[]);_.each(_.range(0,8),function(b){k=Bridge._generateClasses(d,["column",l,b]);var n=["data-level='"+b+"'"];q=Bridge._getTag(a,k);f+=Bridge._openTag(q,a,k,n);var h=Bridge._generateClasses(d,["field",l,b]),g=Bridge._getTag(a,
h);0!==b&&b===c&&h.push("selected");b>=e?h.push("enabled"):(h.push("disabled"),n.push("disabled"));f+=Bridge._openTag(g,a,h,n);0!==b&&(f+=b);f+=Bridge._closeTag(g);f+=Bridge._closeTag(q)},this);l="calls";m=Bridge._generateClasses(d,["row",l]);g=Bridge._getTag(a,m);f+=Bridge._openTag(g,a,m,[]);_.each(Bridge.callOrder.slice().reverse(),function(e){var n=Bridge.isStrain(e)?Bridge.calls[e].html:Bridge.calls[e].text;k=Bridge._generateClasses(d,["column",l,e]);var h=Bridge.isStrain(e)?c+e:e,g=["data-suit='"+
e+"'","data-call='"+h+"'"];q=Bridge._getTag(a,k);f+=Bridge._openTag(q,a,k,g);var m=Bridge._generateClasses(d,["field",l,e]),t=Bridge._getTag(a,m);c&&Bridge.isStrain(e);b[h]?m.push("enabled"):(m.push("disabled"),g.push("disabled"));f+=Bridge._openTag(t,a,m,g);f+=n;f+=Bridge._closeTag(t);f+=Bridge._closeTag(q)},this);var f=f+Bridge._closeTag(g),h="",m=Bridge._generateClasses(d,["row","utilities"]),g=Bridge._getTag(a,m),h=h+Bridge._openTag(g,a,m,[]),k=[],q="",l="calls";_.each([{field:"allpass",text:"All Pass",
call:"allpass",bid:"p"},{field:"skip"},{field:"undo",text:"Undo",call:"undo",bid:"u"},{field:"skip"},{field:"reset",text:"Reset",call:"reset",bid:"u"}],function(c){k=Bridge._generateClasses(d,["column",l,c.field]);q=Bridge._getTag(a,k);var e=["data-bid='"+c.call+"'","data-call='"+c.call+"'"],f="skip"!==c.field?"colspan=2":"colspan=1";e.push(f);if(a.show[c.field]&&"skip"!==c.field){h+=Bridge._openTag(q,a,k,e);var f=Bridge._generateClasses(d,["field",l,c.field]),g=Bridge._getTag(a,f);b[c.bid]?f.push("enabled"):
(f.push("disabled"),e.push("disabled"));h+=Bridge._openTag(g,a,f,e);h+=c.text;h+=Bridge._closeTag(g)}else h+="<"+q+" "+f+">";h+=Bridge._closeTag(q)},this);h+=Bridge._closeTag(g);return Bridge._generateHTMLModule(a,"",f,h)};
...
l);var f=["data-bid='"+e.call+"'","data-call='"+e.call+"'"];if("skip"!==e.field){d+=Bridge._openTag(m,a,l,f);var g=Bridge._generateClasses(c,["field","calls",e.bid]);b[e.bid]?g.push("enabled"):(g.push("disabled"),f.push("disabled"));var h=Bridge._getTag(a,g);d+=Bridge._openTag(h,a,g,f);d+=e.text;d+=Bridge._closeTag(h)}else d+="<"+m+"/>";d+=Bridge._closeTag(m)},this);var d=d+Bridge._closeTag(f),g="";_.each(_.range(1,8),function(d){var e=Bridge._generateClasses(c,["row","level",d]),f=Bridge._getTag(a,
e);g+=Bridge._openTag(f,a,e,[]);_.each(Bridge.callOrder.slice().reverse(),function(e){if(Bridge.isStrain(e)){var f=d+e,h=d+Bridge.calls[e].html;l=Bridge._generateClasses(c,["column","calls",d,e]);var p=["data-level='"+d+"'","data-bid='"+e+"'","data-call='"+f+"'"];m=Bridge._getTag(a,l);g+=Bridge._openTag(m,a,l,p);e=Bridge._generateClasses(c,["field","calls",d,e]);var q=Bridge._getTag(a,e);b[f]?e.push("enabled"):(e.push("disabled"),p.push("disabled"));g+=Bridge._openTag(q,a,e,p);g+=h;g+=Bridge._closeTag(q);
g+=Bridge._closeTag(m)}},this);g+=Bridge._closeTag(f)},this);var h="",e=Bridge._generateClasses(c,["row","utilities"]),f=Bridge._getTag(a,e),h=h+Bridge._openTag(f,a,e,[]),l=[],m="",e=[{field:"allpass",text:"All Pass",call:"allpass",bid:"p"},{field:"skip"},{field:"undo",text:"Undo",call:"undo",bid:"u"},{field:"skip"},{field:"reset",text:"Reset",call:"reset",bid:"u"}];_.each(e,function(d){l=Bridge._generateClasses(c,["column","calls",d.field]);m=Bridge._getTag(a,l);var e=["data-bid='"+d.call+"'","data-call='"+
d.call+"'"];if(a.show[d.field]&&"skip"!==d.field){h+=Bridge._openTag(m,a,l,e);var f=Bridge._generateClasses(c,["field","calls",d.field]),g=Bridge._getTag(a,f);b[d.bid]?f.push("enabled"):(f.push("disabled"),e.push("disabled"));h+=Bridge._openTag(g,a,f,e);h+=d.text;h+=Bridge._closeTag(g)}else h+="<"+m+"/>";h+=Bridge._closeTag(m)},this);h+=Bridge._closeTag(f);return Bridge._generateHTMLModule(a,d,g,h)};
