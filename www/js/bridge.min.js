...
"cards";if(a.show[g]){l=Bridge._generateClasses(c,["column",g,b]);m=Bridge._getTag(a,l);n+=Bridge._openTag(m,a,l,[]);var d=Bridge._generateClasses(c,["field",g,b]),k=Bridge._getTag(a,d),p=["data-suit='"+b+"'"],h="",q=0;_.each(Bridge.rankOrder,function(e){if(this.cards[b][e]){var f=this.showAsX[b][e]?"x":e;e=this.showAsX[b][e]?"x":Bridge.ranks[f].html;q++;d=Bridge._generateClasses(c,["field",g,b,f]);d.push(c+"-field-"+g+"-"+r);k=Bridge._getTag(a,d);h+=Bridge._openTag(k,a,d,["data-suit='"+b+"'","data-rank='"+
f+"'","data-card='"+b+f+"'","data-card-order='"+(14*(3-Bridge.suits[b].index)+("x"===f?13:12-Bridge.ranks[f].index))+"'"]);a.show.text&&(h+=e);h+=Bridge._closeTag(k);r++}},this);0===q&&a.show.emptySuit&&a.show.text&&(h+="-");n+=h;n+=Bridge._closeTag(m)}n+=Bridge._closeTag(f)},this);a.show.countInContent&&(p="count",h=Bridge._generateClasses(c,["field",p]),k=Bridge._getTag(a,h),q=["data-"+p+"='"+this.get(p)+"'"],n+=Bridge._openTag(k,a,h,q),n+="("+this.get(p)+")",n+=Bridge._closeTag(k));d=Bridge._generateHTMLModule(a,
d,n,"");a.containerID&&Bridge.embedHTML(a.containerID,d);!b&&a.registerChangeHandler&&(h=a.idPrefix+"-"+a.prefix,k="hand:changed."+h,$(document).off(k),$(document).on(k,{config:_.cloneDeep(a),hand:this,id:h,event:k,direction:this.getDirection()},function(a,b){if(b===a.data.hand)if(0===$("#"+a.data.id).length){var c=a.data.event;$(document).off(c)}else b.toHTML(a.data.config,!0)}));return d};
...
Bridge.getCardHTML=function(a){if(2!==a.length)return a;var b=a[1].toLowerCase();a=a[0].toLowerCase();"x"!==b&&Bridge._checkRank(b);Bridge._checkSuit(a);var c="",c="x"===b?c+b:c+Bridge.ranks[b].html;return c+=Bridge.suits[a].html};Bridge.getCardHTML2=function(a){if(2!==a.length)return a;Bridge._checkSuit(a[0]);Bridge._checkRank(a[1]);return Bridge.suits[a[0]].html+Bridge.ranks[a[1]].html};
