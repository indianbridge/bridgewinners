...
BW.create = new function() {
  ...
  this.setupClickHandlers = function() {
    ...
    $(document).on("tap", "#create-reset-button.enabled", function(e) {
      self.deal.getHand('s').clearCards();
    });
    // continue clicked
    $(document).on("tap", "#create-continue-button.enabled", function(e) {
      alert("continue clicked");
    });
    // Deal changed.
    var eventName = Bridge.getEventName([self.dealID, Bridge.CONSTANTS.changedEventName]);
  ...
  this.updateStatus = function() {
    if (!this.deal) return;
    var currentHand = this.deal.getHand('s');
    var count = currentHand.getCount();
    if (count > 0) {
      $("#create-reset-button").removeClass("disabled").addClass("enabled");
    } else {
      $("#create-reset-button").removeClass("enabled").addClass("disabled");
    }
    if (count == 13) {
  ...
  this.load = function() {
    ...
    var deal = new Bridge.Deal(self.dealID);
    var savedDeal = localStorage.getItem(self.savedDealName);
    if (savedDeal) {
      deal.fromJSON(JSON.parse(savedDeal));
    }
    this.deal = deal;
...
BW.page = new function() {
  ...
  this.setupClickHandlers = function() {
    var self = this;
    // Menu item change
    $(document).on("tap", ".footer-outer-container .menu-item", function(e) {
  ...
  this.setActiveTab = function(item) {
    console.log("set active tab to " + item);
    this.activeTab = item;
...
BW.user = new function() {
  ...
  this.loadAccount = function() {
    ...
    $("[data-role='section'][data-name='main-page']").show();
  };
